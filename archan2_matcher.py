...
def reshape_image(image):
	return cv2.resize(image, dsize=(227, 227), interpolation=cv2.INTER_CUBIC)
...
def make_data(image, character):
	# make the data format correct for Tensorflowi
	try:
...
def load_session(batch_size, num_classes, character):
	...
	sess = tf.Session()
	saver = tf.train.import_meta_graph(os.path.join(main_dir, 'tmp/finetune_alexnet/chkpt/'+str(character)+'_model_epoch20.ckpt.meta'))
	saver.restore(sess, os.path.join(main_dir,'tmp/finetune_alexnet/chkpt/'+str(character)+'_model_epoch20.ckpt'))
...
###############################################################################################
'''The stupid characterness score calculator'''
###############################################################################################
def stupid_characterness_score(image, character):
	...
	with tf.Session() as sess:
  		saver = tf.train.import_meta_graph('/home/ray/finetune_alexnet_with_tensorflow/tmp/finetune_alexnet/chkpt/model_epoch30.ckpt.meta')
  		saver.restore(sess,'/home/ray/finetune_alexnet_with_tensorflow/tmp/finetune_alexnet/chkpt/model_epoch30.ckpt')
  		sess.run(tf.global_variables_initializer())
  		#graph = tf.get_default_graph()
...
def find_alignment(image, region, word, cost_of_space):
	...
	for i in range(len(annotation)):
		...
		rough_estimates_begin.append(max(0,i*average_width - average_width/2))
		rough_estimates_end.append(min((i+1)*average_width-1+average_width/2, altered_region.shape[1]-1))
	...
	save_vertice_images(altered_region, rough_estimates_begin, rough_estimates_end, 990)
	# initialize an emnpty array of scores
	val_matrix = np.zeros((len(annotation), altered_region.shape[1], altered_region.shape[1]))
	##############################################################################################################
	################################### DIVERGE TO MORNING IDEA ##################################################
	count_id = 0
	all_chars = list(string.ascii_uppercase+string.ascii_lowercase)+[str(x) for x in range(10)]
	char_scores_indv = np.zeros((len(annotation), len(all_chars), altered_region.shape[1], altered_region.shape[1]))
	for _id in range(len(annotation)):
		for char_id in range(62):
			...
			print 'loaded session for character: '+str(char_id)
			for j in range(first_index,last_index-1):
                        	...
                        	for k in range(j+1,last_index):
	                                ...
	                                new_score = smart_characterness_score(altered_image, all_chars[char_id], sess, score)
				
					char_scores_indv[_id, char_id,j,k] = new_score
	np.save('char_scores.npy', char_scores_indv)
	##############################################################################################################
	##############################################################################################################
	# now go ahead and fill up the above val matrix
	...
	for _id in range(len(annotation)):
		...
		if _id >= 0:
			...
			keep_prob = tf.placeholder(tf.float32)
		sess, score = load_session(batch_size=1, num_classes=2, character=char_id)
		print 'loaded session for character: '+annotation[_id]
		########################################################################################################
		for j in range(first_index,last_index-1):
			# loop through all range of widths
			for k in range(j+1,last_index):
		...
		print begin_alignment[-1], end_alignment[-1]
...
def save_vertice_images(image, begin_a, end_a, count):
	...
	for i in range(len(begin_a)):
		...
		cv2.imwrite('./sample'+str(count)+'_'+str(i)+'.jpg', I)
	return None
...
for i in randomized_keys:
	...
	if len(annotation[i]['vertices']) == 4 and annotation[i]['name'] is not None and len(annotation[i]['name'])>2:
		...
		#break
		cv2.imwrite('./sample'+str(count)+'.jpg', im)
