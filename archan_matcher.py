...
#main_dir = '/mnt/nfs/work1/elm/ray/finetune_alexnet_with_tensorflow/'

main_dir = '/mnt/nfs/work1/elm/ray/alexnet_with_tensorflow_map_data/'
...
def find_alignment(image, region, word, cost_of_space):
	...
	for _id in range(len(annotation)):
		...
		# load binary classifier
        	try:
                	char_id = string.ascii_lowercase.index(annotation[_id])
        	except:
                	char_id = range(10).index(int(annotation[_id]))+26
		if _id >= 0:
			tf.reset_default_graph()
			# necessary placeholders for tensorflow sessions
			global x
