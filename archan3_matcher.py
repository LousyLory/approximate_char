...
def stupid_characterness_score(image, character):
	...
	with tf.Session() as sess:
  		saver = tf.train.import_meta_graph('/home/ray/finetune_alexnet_with_tensorflow/tmp/finetune_alexnet/chkpt/model_epoch20.ckpt.meta')
  		saver.restore(sess,'/home/ray/finetune_alexnet_with_tensorflow/tmp/finetune_alexnet/chkpt/model_epoch20.ckpt')
  		sess.run(tf.global_variables_initializer())
  		#graph = tf.get_default_graph()
...
def save_vertice_images(image, begin_a, end_a, count):
	...
	for i in range(len(begin_a)):
		...
		cv2.imwrite('./sample_char_loc'+str(count)+'_'+str(i)+'.jpg', I)
	return None
...
for i in randomized_keys:
	...
	if len(annotation[i]['vertices']) == 4 and annotation[i]['name'] is not None and len(annotation[i]['name'])>2:
		...
		#break
		cv2.imwrite('./sample_char_loc'+str(count)+'.jpg', im)
