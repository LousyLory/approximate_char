...
main_dir = '/mnt/nfs/work1/elm/ray/finetune_vgg16_with_tensorflow/'
...
from vgg16 import VGG16 as AlexNet
...
def get_crop(Img, V, fulcrum):
    ...
    '''
    V = np.asarray(V)
    rowmin = int(min(V[:,1]))#-5, 0)
    rowmax = int(max(V[:,1]))#+5, Img.shape[0])
    colmin = int(min(V[:,0]))
    colmax = int(max(V[:,0]))
...
#############################################################################################
def load_session(batch_size, num_classes, character, iteration=20):
	...
	sess = tf.Session()
	saver = tf.train.import_meta_graph(os.path.join(main_dir, 'tmp/finetune_alexnet/chkpt/'+str(character)+'_model_epoch'+str(iteration)+'.ckpt.meta'))
	saver.restore(sess, os.path.join(main_dir,'tmp/finetune_alexnet/chkpt/'+str(character)+'_model_epoch'+str(iteration)+'.ckpt'))
...
#########################################################################################
def find_alignment(image, region, word, cost_of_space, ite=20):
	...
	for i in range(len(annotation)):
		...
		rough_estimates_begin.append(max(0,i*average_width - average_width/1.5))
		rough_estimates_end.append(min((i+1)*average_width-1+average_width/1.5, altered_region.shape[1]-1))
	...
	for _id in range(len(annotation)):
		...
		if _id > 0:
			first_index = max(rough_estimates_begin[_id], end_alignment[-1])
		else:
		...
		if _id >= 0:
			...
			#y = tf.placeholder(tf.float32, [None, 36])
			keep_prob = tf.placeholder(tf.float32)
		sess, score = load_session(batch_size=1, num_classes=2, character=char_id, iteration=ite)
		print 'loaded session for character: '+annotation[_id]+' with ID: '+str(char_id)
		########################################################################################################
		for j in range(max(0,first_index-3),last_index-1):
			# loop through all range of widths
			for k in range(j+1,min(last_index+10, altered_region.shape[1])):
	...
	#np.save('val_mat_map.npy', val_matrix)
	
	return altered_region, begin_alignment, end_alignment, shift_matrix, _angle, val_matrix
	#return alignment

###########################################################################################
'''Grow region along height only'''
###########################################################################################
def get_vector_and_midpoint(vertices):
	unit_vector = vertices[0]-vertices[1]
        unit_vector = unit_vector/np.linalg.norm(unit_vector)
        mid_point = (vertices[0]+vertices[1])/2.0
	distance = np.linalg.norm(vertices[0]-vertices[1])/2.0
	return unit_vector, mid_point, distance

def get_new_vertices(unit_vector, midpoint, distance):
	new_vertex = np.ceil(midpoint-unit_vector*distance)
	return new_vertex

def grow_height(vertices, rate):
	unit_vector1, midpoint1, distance1 = get_vector_and_midpoint([np.array(vertices[0]), np.array(vertices[3])])
	unit_vector2, midpoint2, distance2 = get_vector_and_midpoint([np.array(vertices[1]), np.array(vertices[2])])
	#print '> original distances: ', distance1, distance2
	#print '> new distances: ', distance1*(rate+100)/100.0, distance2*(rate+100)/100.0
	vertices[0] = list(get_new_vertices(unit_vector1, midpoint1, -distance1*(rate+100)/100.0))
	vertices[3] = list(get_new_vertices(unit_vector1, midpoint1, +distance1*(rate+100)/100.0))
	vertices[1] = list(get_new_vertices(unit_vector2, midpoint2, -distance2*(rate+100)/100.0))
	vertices[2] = list(get_new_vertices(unit_vector2, midpoint2, +distance2*(rate+100)/100.0))
	return vertices

def grow_width(vertices, rate=0):
	offset = 5
	unit_vector1, midpoint1, distance1 = get_vector_and_midpoint([np.array(vertices[0]), np.array(vertices[1])])
	unit_vector2, midpoint2, distance2 = get_vector_and_midpoint([np.array(vertices[3]), np.array(vertices[2])])
	vertices[0] = list(get_new_vertices(unit_vector1, midpoint1, -distance1*(rate+100)/100.0-offset))
	vertices[1] = list(get_new_vertices(unit_vector1, midpoint1, +distance1*(rate+100)/100.0+offset))
        vertices[3] = list(get_new_vertices(unit_vector2, midpoint2, -distance2*(rate+100)/100.0-offset))
        vertices[2] = list(get_new_vertices(unit_vector2, midpoint2, +distance2*(rate+100)/100.0+offset))
	return vertices
...
def save_vertice_images(image, begin_a, end_a, count):
	...
	for i in range(len(begin_a)):
		...
		cv2.imwrite('./sample_char_loc_vgg16'+str(count)+'_'+str(i)+'.jpg', I)
	return None
...
dict_ = {}
# THE MAIN LOOP
annotation = np.load('../annotations/current/'+sys.argv[1]+'.npy').item()
#image = mpimg.imread('../faster-rcnn_backup/DataGeneration/maps_red//D0006-0285025.tiff')
image = cv2.imread('../faster-rcnn_backup/DataGeneration/maps_red/'+sys.argv[1]+'.tiff')
#randomized_keys = np.random.permutation(annotation.keys())
randomized_keys = annotation.keys()
#count = 3
count = 0
#for i in randomized_keys:
#	if lc.represent(annotation[i]['name']).encode('ascii','ignore') == 'INDIANA':
#		print i
#'''
for i in randomized_keys:
	i = 223
	...
	if len(annotation[i]['vertices']) == 4 and annotation[i]['name'] is not None and len(annotation[i]['name'])>=2:
		...
		#if annotation[i]['name'] == 'Perryville':
		#	print count, i
		# logging prints
		#
		print 'segmenting ',annotation[i]['name']
		# compute the segmentation
		im, begin_a, end_a, shift_matrix, _angle, val_matrix = find_alignment(image,annotation[i]['vertices'], annotation[i]['name'], 1, ite=25)
		...
		#break
		cv2.imwrite('./sample_char_loc_vgg16'+str(count)+'.jpg', im)
		save_vertice_images(im, begin_a, end_a, count)
		#
		count += 1
	else:
		if len(annotation[i]['vertices']) == 4 and annotation[i]['name'] is not None and len(annotation[i]['name'])==1:
			annotation[i]['name'] = lc.represent(annotation[i]['name']).encode('ascii','ignore')
			if len(annotation[i]['name']) is not 0:
				len_dict = len(dict_)
				dict_[len_dict+1] = {'vertices': np.array(annotation[i]['vertices']), 'name': annotation[i]['name']}
	if i > 3:
		break
#'''
...
#cv2.imwrite('plotted_image'+sys.argv[1]+'.jpg', image)
#np.save('save_dir/'+sys.argv[1]+'.npy', dict_)
'''
if im is not None:
	print begin_a, end_a
