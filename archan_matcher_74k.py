...
def find_alignment(image, region, word, cost_of_space):
	...
	for i in range(len(annotation)):
		...
		rough_estimates_begin.append(max(0,i*average_width - average_width/4))
		rough_estimates_end.append(min((i+1)*average_width-1+average_width/4, altered_region.shape[1]-1))
	...
	for _id in range(len(annotation)):
		...
		for j in range(first_index,last_index-1):
			...
			for k in range(j+1, last_index):#altered_region.shape[0]/4,last_index): ##################USED PIA'S IDEA
				...
				if annotation[_id] == 'space':
					...
					#print 'altered image_shape: ',altered_image.shape
...
def save_vertice_images(image, begin_a, end_a, count):
	...
	for i in range(len(begin_a)):
		...
		cv2.imwrite('./sample_char_loc_74k'+str(count)+'_'+str(i)+'.jpg', I)
	return None
...
for i in randomized_keys:
	...
	if len(annotation[i]['vertices']) == 4 and annotation[i]['name'] is not None and len(annotation[i]['name'])>2:
		...
		#break
		cv2.imwrite('./sample_char_loc_74k'+str(count)+'.jpg', im)
